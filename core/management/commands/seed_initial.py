"""
Commande : python manage.py seed_initial

Alimente la base avec les données initiales ENSMG (exercices 2022-2026,
référentiels, achats, dons, dotations avec impact stock réel).

Idempotente : ne fait rien si des exercices existent déjà en base.
Utilisée au démarrage sur Render (après les migrations).
"""
import datetime
...
class Command(BaseCommand):
    help = "Seed initial ENSMG — données 2022-2026 (idempotent)"

    def handle(self, *args, **options):
        from core.models import Exercice

        if Exercice.objects.exists():
            self.stdout.write(self.style.WARNING(
                "Base deja alimentee (exercices detectes). Seed ignore."
            ))
            return

        self.stdout.write("Demarrage du seed initial ENSMG...")
        try:
            with transaction.atomic():
                self._run()
            self.stdout.write(self.style.SUCCESS("Seed initial termine avec succes."))
        except Exception as exc:
            self.stderr.write(self.style.ERROR(f"Erreur seed : {exc}"))
            raise
    ...
    def _run(self):
        ...
        if not User.objects.filter(is_superuser=True).exists():
            User.objects.create_superuser("admin", "admin@ensmg.sn", "admin1234")
        ...
        for annee, statut in [(2022,"CLOS"),(2023,"CLOS"),(2024,"CLOS"),(2025,"CLOS"),(2026,"OUVERT")]:
            ...
            exos[annee] = e
        self.stdout.write(f"  {len(exos)} exercices crees")
        ...
        self.stdout.write("  Referentiels crees (depots, services, fournisseurs, donateurs)")
        ...
        self.stdout.write("  Plan comptable et catalogue crees")
        ...
        self.stdout.write(f"  {len(mat)} matieres creees")

        # 11. Transactions
        self._seed_transactions(mat, depots, services, frs, dons_ref)

    # ------------------------------------------------------------------
    def _seed_transactions(self, mat, depots, services, frs, dons_ref):
        ...
        def don(donateur, depot, date, lignes):
            ...
            # Signal absent pour LigneDon -> creation manuelle des mouvements
            ...
            for ligne in dn.lignes.select_related("matiere").all():
                ...
                MouvementStock.objects.get_or_create(
                    ...
                    defaults=dict(
                        type="ENTREE", date=aware_dt, exercice=exo,
                        matiere=ligne.matiere, depot=depot,
                        quantite=ligne.quantite,
                        cout_unitaire=ligne.prix_unitaire or D("0"),
                        reference=dn.code or "",
                        commentaire=f"Entree stock - Don {dn.code}",
                    ),
                )
            return dn
        ...
        # ---- 2022 ----
        achat(frs["DITEX"], MAGCEN, d(2022,1,15), [("ORD-PORT",15,580000),("ORD-DESK",5,450000),("IMP-LASER",4,145000),("VIDEO-PROJ",3,245000),("SWITCH-24",3,185000)], tva=True)
        achat(frs["BUREAUSN"], MAGCEN, d(2022,2,10), [("BUREAU-STD",25,75000),("CHAISE-DIR",40,32000),("CHAISE-VISI",10,18000),("ARMOIRE-MET",20,115000),("CLIM-15",5,300000),("ONDULEUR-1K",8,45000)])
        achat(frs["PAPMOD"], MAGCEN, d(2022,3,5), [("PAPI-A4",300,3200),("STYLO-BX",500,1800),("CAHIER-100",200,1400),("CHEM-CART",150,600),("ENVEL-A4",100,200)])
        achat(frs["SAHELINF"], MAGCEN, d(2022,4,20), [("TONER-HP",20,24000),("CART-ENC",30,14500),("CLE-USB-32",50,8500)])
        achat(frs["CLEANSER"], MAGCEN, d(2022,5,8), [("DETERG-5L",60,2400),("DESINFECT",40,2800),("JAVEL-5L",30,1800)])
        don(dons_ref["UNESCO"], MAGCEN, d(2022,6,15), [("ORD-PORT",5,600000),("VIDEO-PROJ",3,260000),("PAPI-A4",100,0)])
        don(dons_ref["BANKMONDE"], MAGCEN, d(2022,9,20), [("IMP-LASER",2,150000),("CAHIER-100",50,0)])
        dotation(MAGCEN,"Direction Generale",services["DG"],d(2022,2,20),[("ORD-PORT",2,580000),("VIDEO-PROJ",1,245000),("BUREAU-STD",5,75000),("CHAISE-DIR",8,32000),("ARMOIRE-MET",2,115000),("CLIM-15",1,300000),("ONDULEUR-1K",1,45000),("PAPI-A4",50,3200),("STYLO-BX",100,1800)])
        dotation(MAGCEN,"Scolarite",services["SCO"],d(2022,3,15),[("ORD-PORT",3,580000),("IMP-LASER",1,145000),("BUREAU-STD",8,75000),("CHAISE-DIR",10,32000),("ARMOIRE-MET",3,115000),("PAPI-A4",100,3200),("STYLO-BX",200,1800),("CAHIER-100",50,1400)])
        dotation(MAGCEN,"Service Informatique",services["INFO"],d(2022,4,25),[("ORD-PORT",4,580000),("IMP-LASER",2,145000),("SWITCH-24",1,185000),("ONDULEUR-1K",2,45000),("TONER-HP",5,24000),("CART-ENC",10,14500),("CLE-USB-32",20,8500)])
        dotation(MAGCEN,"Bibliotheque",services["BIB"],d(2022,6,20),[("ORD-PORT",2,580000),("IMP-LASER",1,145000),("ORD-DESK",2,450000),("BUREAU-STD",5,75000),("CHAISE-DIR",10,32000),("ARMOIRE-MET",2,115000),("PAPI-A4",50,3200),("CAHIER-100",30,1400)])
        dotation(MAGCEN,"Laboratoire de Geologie",services["LABGEO"],d(2022,8,10),[("ORD-PORT",2,580000),("VIDEO-PROJ",2,245000),("BUREAU-STD",5,75000),("CHAISE-DIR",5,32000),("ARMOIRE-MET",3,115000),("TONER-HP",5,24000),("PAPI-A4",40,3200)])
        dotation(MAGCEN,"Laboratoire des Mines",services["LABMIN"],d(2022,10,5),[("ORD-PORT",2,580000),("VIDEO-PROJ",1,245000),("BUREAU-STD",2,75000),("CHAISE-DIR",5,32000),("ARMOIRE-MET",2,115000),("PAPI-A4",30,3200),("CAHIER-100",30,1400)])
        dotation(MAGCEN,"Service Technique",services["TECH"],d(2022,11,15),[("ORD-PORT",1,580000),("CLIM-15",1,300000),("ONDULEUR-1K",2,45000),("PAPI-A4",20,3200),("DETERG-5L",20,2400),("JAVEL-5L",10,1800)])
        self.stdout.write("  Exercice 2022 OK")

        # ---- 2023 ----
        achat(frs["DITEX"], MAGCEN, d(2023,1,18), [("ORD-PORT",12,620000),("ORD-DESK",4,480000),("IMP-LASER",4,155000),("VIDEO-PROJ",3,260000),("ONDULEUR-1K",5,47000)], tva=True)
        achat(frs["BUREAUSN"], MAGCEN, d(2023,2,15), [("BUREAU-STD",15,80000),("CHAISE-DIR",30,35000),("CHAISE-VISI",10,19000),("ARMOIRE-MET",10,120000),("CLIM-15",3,320000)])
        achat(frs["PAPMOD"], MAGCEN, d(2023,3,8), [("PAPI-A4",250,3300),("STYLO-BX",400,1900),("CAHIER-100",150,1500),("CHEM-CART",100,620),("ENVEL-A4",80,220)])
        achat(frs["SAHELINF"], MAGCEN, d(2023,4,12), [("TONER-HP",18,25000),("CART-ENC",25,15000),("CLE-USB-32",35,9000)])
        achat(frs["CLEANSER"], MAGCEN, d(2023,5,20), [("DETERG-5L",45,2500),("DESINFECT",35,2900),("JAVEL-5L",25,1900)])
        don(dons_ref["GIZ"], MAGCEN, d(2023,8,10), [("ORD-PORT",4,640000),("IMP-LASER",2,160000),("SWITCH-24",2,190000)])
        dotation(MAGCEN,"Ressources Humaines",services["RH"],d(2023,2,25),[("ORD-PORT",2,620000),("BUREAU-STD",3,80000),("CHAISE-DIR",5,35000),("ARMOIRE-MET",1,120000),("PAPI-A4",30,3300),("STYLO-BX",50,1900)])
        dotation(MAGCEN,"Comptabilite et Finances",services["CPT"],d(2023,3,20),[("ORD-PORT",2,620000),("IMP-LASER",1,155000),("BUREAU-STD",4,80000),("CHAISE-DIR",8,35000),("ARMOIRE-MET",2,120000),("PAPI-A4",50,3300),("STYLO-BX",100,1900),("CAHIER-100",30,1500)])
        dotation(MAGCEN,"Service Informatique",services["INFO"],d(2023,5,15),[("ORD-PORT",3,620000),("IMP-LASER",1,155000),("SWITCH-24",1,190000),("ONDULEUR-1K",2,47000),("TONER-HP",5,25000),("CART-ENC",10,15000),("CLE-USB-32",15,9000)])
        dotation(MAGCEN,"Laboratoire de Geologie",services["LABGEO"],d(2023,7,20),[("ORD-PORT",2,620000),("ORD-DESK",2,480000),("BUREAU-STD",5,80000),("CHAISE-DIR",5,35000),("ARMOIRE-MET",2,120000),("PAPI-A4",30,3300),("STYLO-BX",50,1900)])
        dotation(MAGCEN,"Laboratoire des Mines",services["LABMIN"],d(2023,9,10),[("ORD-PORT",3,620000),("ORD-DESK",2,480000),("BUREAU-STD",3,80000),("CHAISE-DIR",5,35000),("ARMOIRE-MET",2,120000),("TONER-HP",3,25000),("PAPI-A4",30,3300)])
        dotation(MAGCEN,"Bibliotheque",services["BIB"],d(2023,11,5),[("ORD-PORT",2,620000),("IMP-LASER",1,155000),("CLIM-15",1,320000),("PAPI-A4",25,3300),("CAHIER-100",50,1500)])
        self.stdout.write("  Exercice 2023 OK")

        # ---- 2024 ----
        achat(frs["DITEX"], MAGCEN, d(2024,1,20), [("ORD-PORT",15,650000),("ORD-DESK",5,500000),("IMP-LASER",5,160000),("VIDEO-PROJ",4,275000),("SWITCH-24",3,200000),("ONDULEUR-1K",8,48000)], tva=True)
        achat(frs["BUREAUSN"], MAGCEN, d(2024,2,14), [("BUREAU-STD",20,82000),("CHAISE-DIR",35,36000),("CHAISE-VISI",15,20000),("ARMOIRE-MET",15,125000),("CLIM-15",5,330000)])
        achat(frs["PAPMOD"], MAGCEN, d(2024,3,10), [("PAPI-A4",350,3400),("STYLO-BX",500,2000),("CAHIER-100",200,1600),("CHEM-CART",150,650),("ENVEL-A4",100,230)])
        achat(frs["SAHELINF"], MAGCEN, d(2024,4,8), [("TONER-HP",20,26000),("CART-ENC",30,15500),("CLE-USB-32",45,9500)])
        achat(frs["CLEANSER"], MAGCEN, d(2024,6,3), [("DETERG-5L",55,2600),("DESINFECT",40,3000),("JAVEL-5L",30,2000)])
        don(dons_ref["UNESCO"], MAGCEN, d(2024,5,20), [("ORD-PORT",3,670000),("VIDEO-PROJ",2,285000),("PAPI-A4",150,0)])
        don(dons_ref["AMBFRANCE"], MAGCEN, d(2024,9,15), [("CAHIER-100",50,0),("CLIM-15",2,340000),("ONDULEUR-1K",3,50000)])
        dotation(MAGCEN,"Direction Generale",services["DG"],d(2024,2,20),[("ORD-PORT",2,650000),("BUREAU-STD",5,82000),("CHAISE-DIR",5,36000),("ARMOIRE-MET",2,125000),("CLIM-15",1,330000),("ONDULEUR-1K",2,48000),("PAPI-A4",60,3400)])
        dotation(MAGCEN,"Scolarite",services["SCO"],d(2024,3,18),[("ORD-PORT",3,650000),("IMP-LASER",1,160000),("BUREAU-STD",5,82000),("CHAISE-DIR",8,36000),("ARMOIRE-MET",3,125000),("PAPI-A4",80,3400),("STYLO-BX",150,2000),("CAHIER-100",50,1600)])
        dotation(MAGCEN,"Service Informatique",services["INFO"],d(2024,5,12),[("ORD-PORT",3,650000),("IMP-LASER",2,160000),("SWITCH-24",1,200000),("ONDULEUR-1K",3,48000),("TONER-HP",5,26000),("CART-ENC",15,15500),("CLE-USB-32",20,9500)])
        dotation(MAGCEN,"Laboratoire de Geologie",services["LABGEO"],d(2024,7,15),[("ORD-PORT",2,650000),("VIDEO-PROJ",2,275000),("BUREAU-STD",5,82000),("CHAISE-DIR",5,36000),("ARMOIRE-MET",2,125000),("PAPI-A4",50,3400),("TONER-HP",3,26000)])
        dotation(MAGCEN,"Ressources Humaines",services["RH"],d(2024,9,20),[("ORD-PORT",2,650000),("BUREAU-STD",4,82000),("CHAISE-DIR",5,36000),("ARMOIRE-MET",2,125000),("CLIM-15",2,330000),("PAPI-A4",40,3400)])
        dotation(MAGCEN,"Comptabilite et Finances",services["CPT"],d(2024,10,8),[("ORD-PORT",2,650000),("IMP-LASER",1,160000),("BUREAU-STD",1,82000),("CHAISE-DIR",5,36000),("ARMOIRE-MET",1,125000),("PAPI-A4",50,3400),("STYLO-BX",100,2000),("CAHIER-100",40,1600)])
        dotation(MAGCEN,"Service Technique",services["TECH"],d(2024,11,10),[("ORD-PORT",1,650000),("ONDULEUR-1K",2,48000),("CLIM-15",1,330000),("PAPI-A4",30,3400),("DETERG-5L",20,2600),("JAVEL-5L",10,2000)])
        self.stdout.write("  Exercice 2024 OK")

        # ---- 2025 ----
        achat(frs["DITEX"], MAGCEN, d(2025,1,22), [("ORD-PORT",12,680000),("IMP-LASER",4,165000),("VIDEO-PROJ",3,285000),("SWITCH-24",2,210000),("ONDULEUR-1K",6,50000)], tva=True)
        achat(frs["BUREAUSN"], MAGCEN, d(2025,2,12), [("BUREAU-STD",18,85000),("CHAISE-DIR",30,37000),("ARMOIRE-MET",12,130000),("CLIM-15",4,340000)])
        achat(frs["PAPMOD"], MAGCEN, d(2025,3,6), [("PAPI-A4",300,3500),("STYLO-BX",400,2100),("CAHIER-100",200,1700),("CHEM-CART",120,680)])
        achat(frs["SAHELINF"], MAGCEN, d(2025,4,10), [("TONER-HP",16,27000),("CART-ENC",22,16000),("CLE-USB-32",30,9800)])
        achat(frs["CLEANSER"], MAGCEN, d(2025,5,15), [("DETERG-5L",40,2700),("DESINFECT",30,3100),("JAVEL-5L",25,2000)])
        don(dons_ref["BANKMONDE"], MAGCEN, d(2025,5,28), [("ORD-PORT",4,700000),("PAPI-A4",100,0)])
        don(dons_ref["GIZ"], MAGCEN, d(2025,10,12), [("CAHIER-100",50,0),("IMP-LASER",2,170000),("SWITCH-24",1,215000)])
        dotation(MAGCEN,"Scolarite",services["SCO"],d(2025,2,20),[("ORD-PORT",2,680000),("IMP-LASER",1,165000),("BUREAU-STD",5,85000),("CHAISE-DIR",8,37000),("ARMOIRE-MET",2,130000),("PAPI-A4",60,3500),("STYLO-BX",100,2100)])
        dotation(MAGCEN,"Direction Generale",services["DG"],d(2025,3,18),[("ORD-PORT",2,680000),("BUREAU-STD",4,85000),("CHAISE-DIR",5,37000),("CLIM-15",1,340000),("PAPI-A4",40,3500)])
        dotation(MAGCEN,"Service Informatique",services["INFO"],d(2025,5,20),[("ORD-PORT",3,680000),("IMP-LASER",1,165000),("SWITCH-24",1,210000),("ONDULEUR-1K",2,50000),("TONER-HP",4,27000),("CART-ENC",10,16000),("CLE-USB-32",10,9800)])
        dotation(MAGCEN,"Comptabilite et Finances",services["CPT"],d(2025,7,10),[("ORD-PORT",2,680000),("BUREAU-STD",3,85000),("CHAISE-DIR",4,37000),("ARMOIRE-MET",2,130000),("PAPI-A4",50,3500),("STYLO-BX",80,2100),("CAHIER-100",40,1700)])
        dotation(MAGCEN,"Laboratoire de Geologie",services["LABGEO"],d(2025,9,8),[("ORD-PORT",2,680000),("VIDEO-PROJ",1,285000),("BUREAU-STD",3,85000),("CHAISE-DIR",4,37000),("ARMOIRE-MET",2,130000),("PAPI-A4",30,3500),("TONER-HP",3,27000)])
        dotation(MAGCEN,"Laboratoire des Mines",services["LABMIN"],d(2025,11,5),[("ORD-PORT",2,680000),("VIDEO-PROJ",1,285000),("BUREAU-STD",3,85000),("CHAISE-DIR",4,37000),("ARMOIRE-MET",2,130000),("PAPI-A4",30,3500),("CAHIER-100",40,1700)])
        self.stdout.write("  Exercice 2025 OK")

        # ---- 2026 ----
        achat(frs["DITEX"], MAGCEN, d(2026,1,20), [("ORD-PORT",15,700000),("ORD-DESK",5,540000),("IMP-LASER",5,170000),("VIDEO-PROJ",4,295000),("SWITCH-24",3,215000),("ONDULEUR-1K",8,52000)], tva=True)
        achat(frs["BUREAUSN"], MAGCEN, d(2026,2,8), [("BUREAU-STD",20,88000),("CHAISE-DIR",35,38000),("CHAISE-VISI",12,22000),("ARMOIRE-MET",15,135000),("CLIM-15",5,350000)])
        achat(frs["PAPMOD"], MAGCEN, d(2026,1,25), [("PAPI-A4",300,3600),("STYLO-BX",450,2200),("CAHIER-100",200,1800),("CHEM-CART",150,700),("ENVEL-A4",100,250)])
        achat(frs["SAHELINF"], MAGCEN, d(2026,3,12), [("TONER-HP",18,28000),("CART-ENC",25,16500),("CLE-USB-32",40,10000)])
        achat(frs["CLEANSER"], MAGCEN, d(2026,3,20), [("DETERG-5L",50,2750),("DESINFECT",35,3200),("JAVEL-5L",28,2100)])
        don(dons_ref["GIZ"], MAGCEN, d(2026,3,28), [("ORD-PORT",3,720000),("PAPI-A4",80,0)])
        dotation(MAGCEN,"Direction Generale",services["DG"],d(2026,2,15),[("ORD-PORT",2,700000),("BUREAU-STD",4,88000),("CHAISE-DIR",5,38000),("ARMOIRE-MET",2,135000),("PAPI-A4",50,3600)])
        dotation(MAGCEN,"Scolarite",services["SCO"],d(2026,3,10),[("ORD-PORT",2,700000),("IMP-LASER",1,170000),("BUREAU-STD",5,88000),("CHAISE-DIR",8,38000),("ARMOIRE-MET",2,135000),("PAPI-A4",60,3600),("STYLO-BX",100,2200)])
        dotation(MAGCEN,"Service Informatique",services["INFO"],d(2026,4,5),[("ORD-PORT",3,700000),("IMP-LASER",1,170000),("SWITCH-24",1,215000),("ONDULEUR-1K",2,52000),("TONER-HP",4,28000),("CART-ENC",10,16500),("CLE-USB-32",15,10000)])
        self.stdout.write("  Exercice 2026 OK")
